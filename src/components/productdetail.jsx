...
const ProductDetails = () => {
  ...
  // const similarproduct=productdata.filter((item)=> item.category.name === data.category.name);
  ...
  return (
    ...
    "><Link to={-1}><i class="ri-arrow-left-line xs:p-0.5 m-1 md:p-1 md:m-1.5 bg-blue-300 rounded-full hover:bg-blue-400 "></i></Link></div>
      {productdetail && (
        <div className="productdetail-wrapper flex bg-white xs:m-0.5 p-0.5 md:p-1 lg:m-2 lg:p-1.5 rounded-lg shadow-lg shadow-blue-300 border-2 border-blue-300">
          <div className="product-images flex flex-col xs:w-1/6">
            {productdetail.images.map((img, idx) => (
              <img
                src={img}
                className="xs:m-0.5 p-0.5 border-2 border-red-100 rounded-md overflow-hidden cursor-pointer "
          ...
          <div className="product-image xs:w-1/2">
            <img
              ...
              alt=""
              className="xs:m-0.5 p-0.5 border-2 border-red-100 rounded-md cursor-pointer"
            />
          </div>
          <div className="product-details xs:w-1/2 xs:p-0.5 m-0.5 sm:m-0.5 sm:p-1 md:p-1.5 lg:m-1">
            <p className="flex justify-between product-details-title font-semibold">
              {productdetail.title} <i className="ri-heart-3-line"></i>
            </p>
            ...
            <p className="flex product-detail-price  xs:mb-0.5">
              ...
              <p className="line-through text-gray-400 xs:pr-2" >₹{productdetail.price + 15}</p>₹
              {productdetail.price}
            </p>
            <p className="product-detail-rating flex items-center xs:mb-0.5">
              <p className="xs:mr-1">Rating :</p>{" "}
              <div className="stars  bg-green-500 flex rounded-sm xs:p-0.5">
            ...
            <div className="button-section xs:mb-0.5 p-1">
              <button className="bg-yellow-400 rounded-sm xs:p-0.5 lg:p-1 rounded-xs mr-2 lg:mr-4" onClick={()=>{BuyNow(productdetail)}}>
